...
AS $function$
  ...
  -- Parse p_distribution_key: if it contains commas, treat as multiple keys
  selected_keys AS (
  ...
  base AS (
    ...
    FROM public.settlements
    WHERE (p_distribution_key IS NULL OR distribution_key IN (SELECT key FROM selected_keys))
  ),
  totals AS (
  ...
  SELECT jsonb_build_object(
    ...
    'periods', (SELECT coalesce(jsonb_agg(jsonb_build_object('distribution', distribution, 'distributionKey', distribution_key, 'rowCount', row_count, 'total', total)), '[]'::jsonb) FROM periods)
  );
$function$;